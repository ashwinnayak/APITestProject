...
//
    ...
    //POST Request to create a single comment record
    public static Response TransactionalEmailPaymentRequest(String xml) {
        RequestSpecification requestSpecification = given().contentType("application/xml").header("SOAPAction","nms:rtEvent#PushEvent");
        requestSpecification.relaxedHTTPSValidation();
        requestSpecification.proxy("http://proxy.jpmchase.net",10443);

        return requestSpecification.log().all().when().body(xml).post( endpointFactory.TestURL);
